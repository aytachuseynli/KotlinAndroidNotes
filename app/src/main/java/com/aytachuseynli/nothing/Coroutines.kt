...
/*
  Why IO dispatcher is better for network tasks than the default dispatcher?
...
1. Why is the IO dispatcher better for network tasks than the default dispatcher?
...
2. Why is the default dispatcher better for CPU-heavy tasks than for network calls?
...
3. What are the different components of a coroutine context?
...
4. Which different synchronization mechanisms are there, and which one is right for each use case?
...
Atomic variables: Used for lightweight synchronization, allowing thread-safe operations without locks. Choosing the right mechanism depends on the specific use case, such as whether you're protecting state, coordinating tasks, or managing access to resources.
5. How to safely handle exceptions in coroutines?
